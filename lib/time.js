...
function buildNumberedVideoTitle(title, orderIndex, totalCount) {
  return `${formatListOrder(orderIndex, totalCount)} - ${title}`;
}

function buildVideoFilename(title, index = null, ext = 'mp4') {
  const base = sanitizeVideoTitle(title);
  if (index == null) return `${base}.${ext}`;
